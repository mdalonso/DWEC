...
document.addEventListener("DOMContentLoaded",()=>{
    document.querySelector("#getUserFetch").addEventListener("click",getFetch);
    document.querySelector("#getUserAsync").addEventListener("click",getFetchAsync);
})

const getFetch=function(){
    const inputId=parseInt(document.querySelector("#userId").value);
    if (isNaN(inputId)) throw new Error("El id debe ser un número entero");

    fetch(`https://jsonplaceholder.typicode.com/users?id=${inputId}`,)
    .then((response)=>{
...
const getFetchAsync=async function(){
    ...
    try{
        ...
        let response=await fetch(`https://jsonplaceholder.typicode.com/users?id=${inputId}`);
