...
function validarCampo(campo, spanError) {
    //Comprobamos si el campo es nombre, en cuyo caso hay que comprobar
    //si cumple con la expresión regular.
    if (campo.id==="nombre"){
        const regex = /^[a-zA-Zá-úÁ-ÚñÑ\s]*$/;
        if (!regex.test(campo.value)) {
            //Establecer un setCustomValidity hace que checkValidity sea false
            //Si setCustomValidity es la cadena vacía, checkValidity será true
            campo.setCustomValidity("Solo se permiten letras y espacios");
        } else {
            campo.setCustomValidity(""); // borrar mensaje si es válido
        }
    } else {
        campo.setCustomValidity(""); // otros campos no tienen validación personalizada
    }
...
function mostrarError(campo, spanError) {
    ...
    //el flag valid indica si el campo cumple con todaas las restricciones o no
    //true si cumple y false si no lo hace.
    //el establecimiento de un setCustomValidation hace que valid sea false
    //set customValidation no tiene ningún flag específico asociado en ValidityState
    //Por eso lo ponemos en el último if
    }else if (!v.valid){
...
function validarGrupoRadio(nombre, spanError) {
    ...
    if (seleccionado){
        ...
        spanError.textContent="Debes seleccionar una opción.";
    }
